...
function puzzleOption(action) {
	 swith(action) {
	 	 case "use" : if ((currentLocation === 3) && (item8 === true) && (use === false) ) {
	 	 	 use = true;
	 	 	 updateDisplay("You have unlocked the hidden room!");
	 	 	 winningMove();
	 	 } else {
	 	 	 updateDisplay("You need a key to unlock this room!");
	 	 }
	 }
}

function winningMove() {
	 updateDisplay("You have ulocked the room of secrets, you have won the game! Your score is" + moveCount);
}